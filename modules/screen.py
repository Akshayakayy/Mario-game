"""This module contains all informations about the screen."""
# IMPORTING MODULES
import sys
import random
from .config import printcc


class Screen(object):
    """This contains all the attributes and functions related to the screen only."""
    scr = []
    ...
    def __init__(self):
        """The constructor, sets the borders, lives, scores, level and cloud images."""
        for i in range(35):
            row = []
            for j in range(150):
        ...
        self.climg1[0][4] = "_"
        self.climg1[1][3] = self.climg1[2][0] = "("
        self.climg1[1][5] = self.climg1[2][6] = ")"
    ...
    def printscr(self):
        """Prints the screen."""
        for row in self.scr:
            rowvar = (''.join(row))[30:110]
            for col in rowvar:
                printcc(col)
            sys.stdout.write('\n')

    def setscene(self):
        """Updates the screen when called by moving it to the right of the buffer."""
